...
export default function ProblemsPage() {
  ...
  const filtered = useMemo(() => {
    return PROBLEMS.filter((p) => {
      ...
      const hitTag = tagArray.length === 0 || tagArray.some((t) => p.tags.includes(t));
        ...
        <div className="p-4">
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {["All", "LeetCode", "HackerRank", "CodeChef", "CodingNinjas"].map((pl) => (
                <button
                  key={pl}
        ...
        <div className="border-l border-slate-800/80 bg-slate-900/50 p-4">
          ...
          <Calendar
            tileClassName={({ date }) => {
              ...
              if (solvedDates.includes(d)) return "bg-emerald-600 text-white rounded-full";
              if (visitedDates.includes(d)) return "bg-rose-600 text-white rounded-full";
              return "";
            }}
          />
          <div className="mt-4 text-xs text-slate-400 space-y-1">
            <div><span className="inline-block w-3 h-3 bg-emerald-600 rounded-full mr-2"></span> Solved</div>
            <div><span className="inline-block w-3 h-3 bg-rose-600 rounded-full mr-2"></span> Visited, not solved</div>
          </div>
        </div>
      </main>
    </div>
